...
Write-Host "$PSScriptRoot\att.dataimport\private\classes\"

$classes = Get-ChildItem -Path $PSScriptRoot\..\att.dataimport\private\classes\*.ps1

foreach( $class in $classes )
{
    $content = Get-Content -Path $class.FullName -Raw

    $null = $sb.AppendLine( "# class file - src\att.dataimport\private\classes\$($class.Name)" )
...
$sb.ToString() | Set-Content -Path "$PSScriptRoot\..\att.dataimport\att.dataimport.psm1"
